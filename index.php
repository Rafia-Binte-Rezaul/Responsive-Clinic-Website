...
doctorDropdown.addEventListener("change", () => {
    const doctorDetails = doctorDropdown.value;
    const doctorId = doctorDetails.split("--")[0];
        ...
        .then(data => {
            if (data.length > 0) {
                data.forEach(slot => {
                    ...
                    option.value = slot.schedule_id + "-->" + slot.date_time;
                    option.textContent = slot.date_time;
                ...
                scheduleDropdown.appendChild(noOption);
            }
        })
        .catch(error => {
            console.error("Error fetching slots:", error);
            const errorOption = document.createElement("option");
            errorOption.textContent = "Error loading slots";
            errorOption.disabled = true;
            scheduleDropdown.appendChild(errorOption);
        });
});
</script>
